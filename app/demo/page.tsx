...
export const metadata: Metadata = {
  ...
  description: "Prófaðu Mín fjármál með tilbúnum gögnum. Þú þarft ekki að stofna aðgang."
};
...
export default async function DemoPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <DemoApp
    ...
    marketSummary={<Suspense fallback={<DataSectionLoading label="Sæki markaðsgögn…" />}><MarketDataSection data={marketData} compact detailsHref="/demo?view=markets" /></Suspense>}
    marketContent={<Suspense fallback={<DataSectionLoading label="Sæki markaðsgögn…" />}><MarketDataSection data={marketData} /></Suspense>}
    realEstateContent={<Suspense fallback={<DataSectionLoading label="Sæki fasteignagögn…" />}><DemoRealEstate data={realEstateData} /></Suspense>}
  />;
}
