...
export async function GET(request: Request) {
  ...
  if (error || !data.user) return Response.json({ error: "Innskráningar er þörf." }, { status: 401, headers });
  const user = data.user;
  const format = new URL(request.url).searchParams.get("format") ?? "json";
  if (format !== "json" && format !== "csv") return Response.json({ error: "Óþekkt skráarsnið." }, { status: 400, headers });
  try {
    ...
    const body = format === "csv" ? transactionsCsv(records.transactions) : JSON.stringify({ exported_at: new Date().toISOString(), currency: "ISK", account: { id: user.id, email: user.email, created_at: user.created_at, deletion_request: user.user_metadata?.account_deletion_request ?? null }, data: records }, null, 2);
    ...
    return Response.json({ error: "Ekki tókst að sækja öll gögnin. Engin ófullgerð skrá var búin til. Reyndu aftur." }, { status: 503, headers });
  }
}
