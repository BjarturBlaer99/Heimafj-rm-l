...
export function AccountDataControls({ requestedAt, supportEmail }: { requestedAt: string | null; supportEmail?: string }) {
  ...
  async function download(format: "csv" | "json") {
    ...
    try {
      ...
      setNotice("Gögnin voru tilbúin og niðurhal hófst.");
    } catch { setNotice("Ekki tókst að flytja út gögnin. Athugaðu innskráningu og reyndu aftur."); }
    finally { setDownloading(null); }
  }
  return <div className={styles.dataControls}>
    <div><h3>Sækja gögnin mín</h3><p>CSV inniheldur allar færslur. JSON inniheldur einnig flokka, áætlanir, reikninga, greiðslur, sparnað og aðgangsupplýsingar. Skrárnar vistast á tækinu þínu.</p>
    ...
    <div><h3>Leiðbeiningar og aðstoð</h3><p>Finndu svör um innflutning, flokkun, reikninga og gögnin þín.</p><div className={styles.dataLinks}><Link href="/help">Opna leiðbeiningar</Link><Link href="/privacy">Um meðferð gagna</Link>{supportEmail ? <a href={`mailto:${supportEmail}`}>Hafa samband</a> : null}</div></div>
    <div id="account-deletion"><h3>Beiðni um eyðingu aðgangs</h3><p>Við eyðingu aðgangs hverfa vistaðar færslur, reikningar, markmið og önnur gögn hans úr virka kerfinu. Sæktu afrit áður en þú óskar eftir eyðingu.</p><p>Beiðnin vistast með aðganginum og þarf afgreiðslu stjórnanda. Hún eyðir engu sjálfkrafa. Þú getur afturkallað hana hér þar til eyðing hefur verið framkvæmd.</p>
      ...
      {supportEmail ? <p>Fyrir aðstoð með beiðnina: <a href={`mailto:${supportEmail}`}>{supportEmail}</a>.</p> : null}
    </div>
  </div>;
}
