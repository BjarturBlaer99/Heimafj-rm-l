...
export function SetupChecklist({ userId, hasTransactions, hasBills, hasGoal }: {
  ...
  return (
    <section data-scroll-reveal="" className={styles.checklist} aria-labelledby="setup-title">
      <header className={styles.heading}><div><h2 id="setup-title">Fyrsti mánuðurinn</h2><p>Byrjaðu á að skrá færslur, yfirfara flokka og setja þér markmið.</p></div><span>{count} af 4 skrefum lokið</span></header>
      <ol className={styles.steps}>
        ...
        <li><span className={styles.status} data-complete={complete[1]} aria-label={complete[1] ? "Lokið" : "Ólokið"}>{complete[1] ? <CheckIcon size={14} aria-hidden="true" /> : "2"}</span><div><h3>Yfirfarðu flokkunina</h3><p>Farðu yfir tillögur að flokkun innfluttra færslna og leiðréttu það sem þarf.</p><div className={styles.links}><Link href="/transactions">Skoða færslur</Link><button type="button" onClick={() => acknowledge("reviewed", !reviewed)} aria-pressed={reviewed}>{reviewed ? "Merkja sem óyfirfarið" : "Ég hef yfirfarið flokkana"}</button></div></div></li>
        <li><span className={styles.status} data-complete={complete[2]} aria-label={complete[2] ? "Lokið" : "Ólokið"}>{complete[2] ? <CheckIcon size={14} aria-hidden="true" /> : "3"}</span><div><h3>Skráðu reikningana þína</h3><p>Skráðu upphæðir og gjalddaga til að sjá hvað bíður greiðslu.</p><div className={styles.links}><Link href="/bills">Opna reikninga</Link>{!hasBills && <button type="button" onClick={() => acknowledge("noBills", !noBills)} aria-pressed={noBills}>{noBills ? "Ég vil skrá reikninga" : "Ég hef enga reikninga að skrá"}</button>}</div></div></li>
        <li><span className={styles.status} data-complete={complete[3]} aria-label={complete[3] ? "Lokið" : "Ólokið"}>{complete[3] ? <CheckIcon size={14} aria-hidden="true" /> : "4"}</span><div><h3>Veldu sparnaðarmarkmið</h3><p>Veldu hvað þú vilt spara mikið og hvenær þú vilt ná markmiðinu.</p><div className={styles.links}><Link href="/savings-goals">Opna sparnað</Link></div></div></li>
      </ol>
      <p className={styles.note}>Ef þú merkir flokkana sem yfirfarna eða velur að sleppa reikningum vistast það aðeins í þessum vafra, fyrir þennan aðgang. <Link href="/help">Leiðbeiningar</Link></p>
    </section>
  );
}
